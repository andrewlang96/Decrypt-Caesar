...
came from an English text, we can say that there is a 60% chance that the prior to encryption, the "ui" was a "th" and that the key used to encrypt it was 1. While this is a substantial improvement over the 1/26 chance of any numbers 0 through 25 being the key, it's still wrong 2 out of 5 times which is far to haigh an error rate if we want the computer to find the key unsupervised. There is also that fact that finding a letter pair with a distance of 11 is the best case scenario, as it represent by far the most common letter pair in English.
